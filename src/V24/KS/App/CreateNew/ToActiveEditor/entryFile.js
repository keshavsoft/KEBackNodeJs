...
const StartFunc = async ({ inEndPointNeeded }) => {
    try {
        ...
        const LocalEndPointNeeded = inEndPointNeeded;
...
const LocalFuncCheckOldEndPoints = ({ inLinesArray, inNewRoute }) => {
    ...
    let LocalFilteredRows = LocalLines.filter((element) => element.startsWith("app.use("));
