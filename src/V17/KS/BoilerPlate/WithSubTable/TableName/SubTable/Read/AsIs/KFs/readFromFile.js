...
let StartFunc = ({ inRowIndex, inKeyName }) => {
    ...
    const LocalinRowIndex = inRowIndex;
    const LocalinKeyName = inKeyName;
    ...
    try {
        ...
        let LoalRowData = JSON.parse(data).find(el => el.pk == LocalinRowIndex);

        if (!LoalRowData) LocalReturnData.KReason = `No Row Data ${LocalinRowIndex}`;

        LocalReturnData.KTF = true;
        LocalReturnData.JsonData = LoalRowData[LocalinKeyName];
    } catch (err) {
